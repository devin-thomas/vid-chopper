...
**Running under Devin:** `references/*.md` paths below are relative to this
skill's directory (`.devin/skills/qt-project/`); read them with the `read`
tool. For the *Documentation lookup* step, the "Qt docs MCP tool" maps to a
Devin MCP server whose name contains `qt-docs` if one is configured, and the
web-fetch fallback maps to Devin's browser tools.
