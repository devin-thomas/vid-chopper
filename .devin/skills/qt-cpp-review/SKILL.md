...
compatibility: Adapted for Devin (also runs on Claude Code, GitHub Copilot, and similar agents).
...
combines deterministic linting with agent-driven deep
analysis across six focused domains.

## Running under Devin (tool mapping)

This skill was authored for Claude Code; the procedure is identical
under Devin with these substitutions:

- **"Launch six parallel subagents"** → run the six Agent missions as
  **child sessions** when you want true parallelism (use the
  `managing-child-sessions` skill — one child per mission, each
  pointed at this repo with its mission text and the Phase 1 lint
  output as context), **or**, for a smaller/faster review, perform the
  six missions yourself as **sequential analysis passes** in this
  session. Either way, all six missions must be completed before
  Phase 3.
- **`Glob` tool** → the `grep` tool (`output_mode: "files_with_matches"`)
  or shell globbing via `exec`.
- **`Bash` tool** → the `exec` tool.
- **Reference paths** (e.g. `references/qt-review-checklist.md`) are
  relative to this skill's directory:
  `.devin/skills/qt-cpp-review/`. Read them with the `read` tool.
- **Running the linter**: invoke it by its repo-relative path (see
  Phase 1). On Windows the interpreter is `python` (there is no
  `python3` on PATH); on Linux/macOS use `python3`.
- This skill is **read-only** — it never edits files, opens PRs, or
...
- Default — review using universal Qt6 C++ rules only.
- `framework` (e.g. "review framework") — also apply Qt
  framework/module development rules (BC, exports, d-pointers, qdoc,
  QML versioning).

## Framework mode detection

If the user's request includes the word "framework", enable
framework mode.

If not, auto-detect by scanning the first
few files in scope for framework signals. If **two or more** of
the following are found, suggest to the user:
"This looks like Qt framework/module code. Want me to also apply
framework-specific rules (BC, exports, qdoc, QML versioning)?"
...
```bash
# Linux/macOS:
python3 .devin/skills/qt-cpp-review/references/lint-scripts/qt_review_lint.py <files...>
# Windows (and any host without python3 on PATH):
python .devin/skills/qt-cpp-review/references/lint-scripts/qt_review_lint.py <files...>
```

Run this through the `exec` tool. The path is relative to the
repository root; adjust if the skill lives elsewhere.
...
### Phase 2: Agent-driven deep analysis (6 review missions)

Run six focused review missions covering the domains below. Under
Devin, either dispatch them as parallel **child sessions** (via the
`managing-child-sessions` skill — name each descriptively, e.g.
"Agent 1: Model Contracts", for progress visibility) or complete them
yourself as sequential analysis passes. The missions are READ-ONLY —
they must never edit or write files.

**Tool-agnostic agent contract**: Each mission described below is
a self-contained review task. Implement each as whatever
subprocess, child session, prompt chain, or analysis pass your tool
supports. The key requirement is that each mission:
