...
`ExportEngine` is the final shared execution API. `std::stop_token` is forwarded into every process
request, while bounded process-output callbacks drive progress and raw diagnostics without introducing
Qt types. Existing Job/result aggregates and lifecycle callbacks remain valid across both adapters.
...
The adapter MUST NOT call `ExportEngine::run` on the UI thread. Export callbacks MUST NOT dereference
...
| Shared planning, execution, and manifests | `src/services/` |
| CLI presentation adapter | `src/cli/` |
| Qt asynchronous adapter | `src/qt/services/` |
| Native, integration, and Qt tests | `tests/` |

The Markdown file is the authoritative internal source. `npm test` in `docs/` verifies required sections,
cross-links, public-contract tokens, and CI routing. No tracked PDF or PDF rendering/freshness machinery is
required. Presentation work for public documentation at `vidchopper.app/docs` is post-1.0.
...
- Changing persisted settings, naming, manifests, or exit codes as refactor cleanup. The shared
  `-progress pipe:1 -nostats` arguments are the intentional command-contract update for adapter parity.
