...
auto main() -> int {
    ...
    // split_quoted_arguments: edge cases
    {
        ...
        test_support::expect_eq(empty_result.size(), size_t {0}, "empty input yields no tokens");

        const auto whitespace_result = split_quoted_arguments("   ");
        test_support::expect_eq(whitespace_result.size(), size_t {0}, "whitespace-only yields no tokens");

        const auto single = split_quoted_arguments("single");
        test_support::expect_eq(single.size(), size_t {1}, "single word yields one token");
        test_support::expect_eq(single[0], std::string {"single"}, "single word content");

        const auto multi_space = split_quoted_arguments("a   b   c");
        test_support::expect_eq(multi_space.size(), size_t {3}, "multiple spaces between tokens");

        const auto quoted_empty = split_quoted_arguments("\"\"");
        test_support::expect_eq(quoted_empty.size(), size_t {0}, "empty quoted string yields no tokens");

        const auto mixed = split_quoted_arguments("before \"quoted part\" after");
        test_support::expect_eq(mixed.size(), size_t {3}, "mixed quoted/unquoted should yield 3 tokens");
