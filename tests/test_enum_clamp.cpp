...
enum LegacyUnscoped { LegacyA, LegacyB };
...
auto main() -> int {
    ...
    test_support::expect_true(
        clamp_to_enum(-5, EncoderKind::HevcNvenc, EncoderKind::Auto) == EncoderKind::Auto,
        "negative raw should fall back to Auto"
    );
    test_support::expect_true(
        clamp_to_enum(2, EncoderKind::HevcNvenc, EncoderKind::Auto) == EncoderKind::HevcNvenc,
        "raw == max_valid should map to that enumerator"
    );
    test_support::expect_true(
        clamp_to_enum(99, EncoderKind::HevcNvenc, EncoderKind::Auto) == EncoderKind::Auto,
        "raw beyond max should fall back to Auto"
    );

    // ContainerMode: 0..2 valid, fallback Source.
    test_support::expect_true(
        clamp_to_enum(1, ContainerMode::Mkv, ContainerMode::Source) == ContainerMode::Mp4,
        "raw 1 should map to Mp4"
    );
    test_support::expect_true(
        clamp_to_enum(3, ContainerMode::Mkv, ContainerMode::Source) == ContainerMode::Source,
        "raw 3 should fall back to Source"
    );

    // SeekMode: 0..1 valid, fallback Accurate.
    test_support::expect_true(
        clamp_to_enum(1, SeekMode::Fast, SeekMode::Accurate) == SeekMode::Fast,
        "raw 1 should map to Fast"
    );
    test_support::expect_true(
        clamp_to_enum(2, SeekMode::Fast, SeekMode::Accurate) == SeekMode::Accurate,
