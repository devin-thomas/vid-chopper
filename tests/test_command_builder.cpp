...
auto main() -> int {
    ...
    test_support::expect_eq(output_path.filename().string(), std::string {"01 - Intro _ Setup.mov"}, "output name should be sanitized and padded");

    const auto auto_encoder = resolve_encoder(settings, EncoderEnvironment {.has_nvidia_gpu = true, .has_hevc_nvenc_encoder = true});
    test_support::expect_eq(auto_encoder.video_codec, std::string {"hevc_nvenc"}, "auto mode should prefer nvenc when available");

    settings.encoder_kind = EncoderKind::X264;
    const auto x264_encoder = resolve_encoder(settings, EncoderEnvironment {.has_nvidia_gpu = true, .has_hevc_nvenc_encoder = true});
    test_support::expect_eq(x264_encoder.video_codec, std::string {"libx264"}, "explicit x264 mode should override nvenc");
