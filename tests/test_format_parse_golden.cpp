...
auto main() -> int {
    // format_millisecond_timecode: exact padding across magnitudes.
    {
        ...
        test_support::expect_eq(
            format_millisecond_timecode(65250), std::string {"00:01:05.250"}, "ms one minute five seconds");
        test_support::expect_eq(
            format_millisecond_timecode(3661001), std::string {"01:01:01.001"}, "ms hour minute second milli");
        test_support::expect_eq(format_millisecond_timecode(360000000),
            std::string {"100:00:00.000"},
            "ms hours overflow two -> three digits");
    }
    ...
    // format_frame_timecode: exact padding and frame component.
    {
        ...
        test_support::expect_eq(
            format_frame_timecode(360000000, fps_30), std::string {"100:00:00:00"}, "frame hours three digits");
    }
    ...
    // zero_padded_index: width zero, width one, exact width, and max u16.
    {
        ...
        test_support::expect_eq(
            zero_padded_index(65535, 3), std::string {"65535"}, "value wider than width stays intact");
    }

    // parse_millisecond_timecode: from_chars overflow and sign/space rejection.
    {
        test_support::expect_true(!parse_millisecond_timecode("99999999999999999999:00:00.000").has_value(),
            "hour segment overflow should fail");
        ...
        test_support::expect_true(
            !parse_millisecond_timecode("00: 5:00.000").has_value(), "interior space should fail");
    }
