...
auto main() -> int {
    ...
    test_support::expect_eq(x264.quality_semantics,
        EncoderQualitySemantics::ConstantRateFactor,
        "x264 must own CRF quality semantics");
    ...
    test_support::expect_eq(nvenc.quality_semantics,
        EncoderQualitySemantics::ConstantQuantizer,
        "NVENC must own CQ quality semantics");
    test_support::expect_eq(nvenc.quality_argument, std::string_view {"-cq"}, "NVENC quality argument");
    test_support::expect_true(encoder_platform_eligible(nvenc, EncoderPlatform::Windows),
        "NVENC should be eligible on Windows");
    test_support::expect_true(encoder_platform_eligible(nvenc, EncoderPlatform::Linux),
        "NVENC should be eligible on Linux");
    test_support::expect_true(!encoder_platform_eligible(nvenc, EncoderPlatform::MacOs),
        "NVENC should not be eligible on macOS");

    const EncoderDescriptor videotoolbox = encoder_descriptor(EncoderKind::HevcVideoToolbox);
    test_support::expect_eq(videotoolbox.display_name,
        std::string_view {"HEVC VideoToolbox"},
        "VideoToolbox display name");
    test_support::expect_eq(videotoolbox.codec_name,
        std::string_view {"hevc_videotoolbox"},
        "VideoToolbox codec name");
    ...
    test_support::expect_eq(
        videotoolbox.quality_argument, std::string_view {"-q:v"}, "VideoToolbox quality argument");
    ...
    test_support::expect_true(!videotoolbox.enabled_in_release,
        "VideoToolbox must remain disabled before the 1.2.0 backend release");
    ...
    const ResolvedEncoder resolved_nvenc = resolve_encoder(
        settings, EncoderEnvironment {.has_nvidia_gpu = true, .has_hevc_nvenc_encoder = true});
    test_support::expect_eq(
        resolved_nvenc.kind, EncoderKind::HevcNvenc, "Auto should preserve legacy NVENC selection");
    ...
    const ResolvedEncoder resolved_videotoolbox = resolve_encoder(
        ExportSettings {.encoder_kind = EncoderKind::HevcVideoToolbox}, EncoderEnvironment {});
    ...
    test_support::expect_true(resolved_videotoolbox.used_fallback,
        "disabled VideoToolbox command construction should mark its fallback");
