        ...
        std::string {R"json(
{
  ...
  "chapters": [
    ...
    test_support::expect_eq(json_result.config.chapters.size(), std::size_t {2}, "JSON should load two chapters");
    test_support::expect_eq(json_result.config.chapters[0].name, std::string {"Opening"}, "JSON chapter name");
    test_support::expect_eq(json_result.config.chapters[1].start_ms, 60000ULL, "JSON chapter start");
    test_support::expect_eq(json_result.config.settings.output_folder_pattern, std::string {"%source%_clips"}, "folder override");
    test_support::expect_eq(json_result.config.settings.naming_pattern, std::string {"%index%_%name%"}, "naming override");
...
chapters:
  ...
  - name: Middle
    ...
    test_support::expect_eq(yaml_result.config.chapters.size(), std::size_t {2}, "YAML should load two chapters");
...
chapters:
  - name: Short
    ...
    test_support::expect_eq(yml_result.config.chapters.front().name, std::string {"Short"}, "YML chapter name");
    ...
    test_support::expect_true(contains(bad_extension.error_message, "unknown chapter config extension"), "extension error");
...
chapters:
  - name: Opening
    ...
    test_support::expect_true(contains(unknown_key.error_message, "unknown chapter key"), "unknown key error");
    test_support::expect_true(contains(unknown_key.error_message, "unknown-key.yaml"), "error should include path");
        ...
        std::string {R"json(
{
  ...
  "chapters": [
    ...
    test_support::expect_true(contains(invalid_time.error_message, "invalid timestamp"), "timestamp error");
...
chapters:
  - name: Backwards
    ...
    test_support::expect_true(contains(invalid_order.error_message, "Chapter end time"), "core validation error");
